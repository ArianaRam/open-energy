...
Acorngroup= m3.groupby(['Acorn_grouped']).count()['LCLid']
...
X= [Summary, [m3.iloc[n,1] for n in range(len(m3))], [m3.iloc[n,4] for n in range(len(m3))]]

#%% [10] split into train test sets


X_train, X_test, y_train, y_test= train_test_split(X[0], X[2], test_size=0.4, stratify= X[2], random_state= 1)
...
plot_tree(mod_dt, feature_names= LCL2.columns, class_names=Acorngroup.index, filled=True)
...
disp= metrics.plot_confusion_matrix(mod_dt, X_test, y_test, 
                                    display_labels= Acorngroup.index, 
                                    cmap= plt.cm.Blues,
                                    normalize=None, 
                                    ax=ax )                        
...
for name, model in models:
	...
	cv_results = cross_val_score(model, X[0], X[2], cv=kfold, scoring='accuracy')
