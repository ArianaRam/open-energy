...
Summary= [np.mean((sel(LCLHfull, n)), axis=0) for n in range(len(LCLHfull.T))]
...
X= [Summary, [masterC.iloc[n,1] for n in range(len(masterC))], [masterC.iloc[n,0] for n in range(len(masterC))]]
...
#%% [11] examine the importance of each predictor: 
